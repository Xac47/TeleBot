...
from config import token, regions
...
def start(message):
    ...
    rkm.add(types.KeyboardButton('author'),types.KeyboardButton('Найти регион'),types.KeyboardButton('Назад'))
...
def menu(message):
    ...
    if message.text == 'author':
        msg = bot.send_message(message.chat.id, '<b>Не твоё дело...</b>', parse_mode='html')
        bot.register_next_step_handler(msg, menu)
        ...
        msg = bot.send_message(message.chat.id, '<b>Выберите нужный блок!</b>', parse_mode='html')
        bot.register_next_step_handler(msg, menu)
...
def get_regions(message):
    ...
    if message.text in regions:
        ...
        msg = bot.send_message(message.chat.id, '<b>Напишите другой регион!</b>', parse_mode='html')
        bot.register_next_step_handler(msg,get_regions)
